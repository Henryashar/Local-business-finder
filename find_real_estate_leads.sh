...
# Scrapes seller-signal and referral-partner businesses from Google Maps
...
RESULTS_DIR="$SCRIPT_DIR/results/real_estate"
COMPLETED_FILE="$SCRIPT_DIR/completed_real_estate_cities.txt"
TEMP_QUERIES="$SCRIPT_DIR/.temp_real_estate_queries_$$.txt"

# Seller signals: businesses that often coincide with or precede a sale
seller_signals=(
    "estate sale company"
    "estate liquidator"
    "moving company"
...
# Referral partners: businesses that work with sellers and can refer them
referral_partners=(
    "probate attorney"
...
# Extract emails from websites; write to separate _emails.csv (raw CSV left intact)
extract_emails_from_websites() {
    ...
    # Do NOT overwrite raw CSV; emails go to _emails.csv only
    if [[ -f "$output_file" ]]; then
        local count=$(wc -l < "$output_file" | tr -d ' ')
        count=$((count - 1))
        print_msg "$GREEN" "  Email-extracted leads saved to: ${output_file}"
...
# Generate query file for a city (seller_signals + referral_partners)
generate_queries() {
    local city=$1
    local test_mode=$2
    ...
    # Combine segments into one list for iteration
    local all_types=("${seller_signals[@]}" "${referral_partners[@]}")
    local types_to_use=("${all_types[@]}")
    
    # In test mode, only use first 2 types
    if [[ "$test_mode" == "true" ]]; then
        types_to_use=("${all_types[@]:0:2}")
    fi
...
run_scraper() {
    ...
    print_msg "$GREEN" "Starting real estate lead scraper for: ${city}"
...
main() {
    ...
    if check_completed "$city"; then
        ...
        if [[ "$force_mode" == "true" ]]; then
            ...
            echo ""
            if [[ ! $REPLY =~ ^[Yy]$ ]]; then
                print_msg "$BLUE" "Skipping. Use a different city or delete the entry from completed_real_estate_cities.txt"
                exit 0
            fi
        fi
        echo ""
    ...
    if run_scraper "$city" "$output_file" "$test_mode"; then
        ...
        # Post-process: extract emails to separate file (raw CSV unchanged)
        if [[ -f "$output_file" ]]; then
            local raw_count=$(wc -l < "$output_file" | tr -d ' ')
            raw_count=$((raw_count - 1))
            print_msg "$GREEN" "  Raw results saved to: ${output_file} (${raw_count} rows)"
